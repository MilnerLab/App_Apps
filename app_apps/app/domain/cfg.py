...
class CFG:
    ...
    T_GA_fwhm: Time = None
    beta_DA: AngularChirp = None              # reference (R) arm chirp, rad/ns^2
    ...
    def update(self, params: SpectralFitParams) -> None:
        ...
        ln2 = math.log(2.0)
        t_tl_fwhm_ns = 4.0 * ln2 / domega_fwhm                        # Eq. (10) at zero GDD: transform-limited duration
        a = 4.0 * ln2 / domega_fwhm**2                                # Eq. (6)

        # --- DA (reference, R) arm: chirp from the xcorr-measured T_DA_fwhm alone ---
        t_da_ns = self.T_DA_fwhm.value(Prefix.NANO)                   # xcorr-measured, given
        phi_da = GDD_SIGN * math.sqrt(max(t_da_ns**2 - t_tl_fwhm_ns**2, 0.0)) / domega_fwhm  # Eq. (11): phi''_DA
        beta_da = -phi_da / (a**2 + phi_da**2)                        # Eq. (9)
        self.beta_DA = AngularChirp(beta_da, time_prefix=Prefix.NANO)

        # --- GA (grating, L) arm: offset from DA by the fitted differential GDD ---
        delta_phi = 2.0 * float(params.theta2)          # Eq. (5): Theta2 = 1/2 * Delta phi'' = 1/2 (phi''_DA - phi''_GA)
        phi_ga = phi_da - delta_phi                      # phi''_GA = phi''_DA - Delta phi''
        beta_ga = -phi_ga / (a**2 + phi_ga**2)           # Eq. (9)
        self.beta_GA = AngularChirp(beta_ga, time_prefix=Prefix.NANO)

        t_ga_fwhm_ns = math.sqrt(t_tl_fwhm_ns**2 + (phi_ga * domega_fwhm)**2)  # Eq. (10): T_GA,FWHM from phi''_GA
        self.T_GA_fwhm = Time(t_ga_fwhm_ns, Prefix.NANO)

        tau_ns = -float(params.theta1)                    # Eq. (5): Theta1 = -tau
        ...
        t_r_ns = t_da_ns / (2.0 * math.sqrt(ln2))
        t_l_ns = t_ga_fwhm_ns / (2.0 * math.sqrt(ln2))

        t_eff_ns = t_r_ns * t_l_ns / math.sqrt(t_r_ns**2 + t_l_ns**2)  # Eq. (17)

        # center frequency at the overlap envelope center t_c, with z_R = 0            # Eq. (20)
        f_center = -tau_ns * (beta_da * t_r_ns**2 + beta_ga * t_l_ns**2) / (4.0 * math.pi * (t_r_ns**2 + t_l_ns**2))
        half_width = (beta_da - beta_ga) / (4.0 * math.pi) * math.sqrt(2.0 * ln2) * t_eff_ns  # Eq. (19)
