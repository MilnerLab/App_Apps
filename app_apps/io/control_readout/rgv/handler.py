...
class RgvHandle(BaseWorkerHandle):
    ...
    **Why this is not a ``MotorizedStageHandle``.** That base class maps its move event
    straight onto ``move_to``, i.e. it reads the event as an ABSOLUTE target. Here the
    event is a relative increment, and the two cannot be reconciled by a hook: routing
    ``RequestRotateRGV`` through the base would command the plate to the increment rather
    than by it, turning a 0.2 degree correction into a move to 0.2 degrees. The spin state
    is the second reason -- a free-running plate has no position to move from, and every
    path here consults that before it commands anything.
    """
